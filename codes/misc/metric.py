...
class MetricManager():
    ...
    def pixel2mm(self, config, points, pspace):
        # points: b, c, 2=(row, column)
        # pspace: b, 4=(raw_row, raw_column, row space, column space)

        mm_points = torch.zeros_like(points)
        pspace = pspace.to(points.device)
        # column: x(mm) = x(pixel) / 224 * 1935 * 0.1(mm/pixel)
        mm_points[:, :, 1] = points[:, :, 1] / config.Dataset.image_size[1] * pspace[:, 1:2] * pspace[:, 3:4]

        # row: y(mm) = y(pixel) / 224 * 2400 * 0.1(mm/pixel)
