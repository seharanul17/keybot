...
class Trainer(object):
    ...
    def test(self, save_manager, test_loader, writer=None):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(test_loader)):
                ...
                for n_hint in range(save_manager.config.Dataset.num_keypoint+1):
                    # 0~max hint
    ...
    # CAUTION::: if metric manager is not None, the post processing is performed
    def forward_batch(self, batch, metric_flag=False, average_flag=True,
                      metric_manager=None, prev_heatmap=None,
                      return_post_processing_pred=False,
                      pseudo_hint_index=None, pseudo_hint_coord=None,
                      pseudohint_full_recon_result=None):

        batch.pseudo_hint_index = pseudo_hint_index
        batch.pseudo_hint_coord = pseudo_hint_coord

        if prev_heatmap is not None:
            batch.prev_heatmap = prev_heatmap
        ...
        with torch.no_grad():
            if metric_manager is None:
                ...
                if pseudohint_full_recon_result is not None and pseudohint_full_recon_result != [[]]:
                    raise()
