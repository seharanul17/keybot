    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            for i in range(batch.label.heatmap.shape[0]):
                if 'pseudo_hint_index' in batch and batch['pseudo_hint_index'] is not None:
                    if batch.pseudo_hint_index[i] is not None and batch.pseudo_hint_index[i] != []:
                        coord = torch.tensor(batch.pseudo_hint_coord[i:i+1]).to(self.device) #(b, k, 2)
                        heatmap = self.heatmap_maker.coord2heatmap(coord) #(1,1,2)
                        batch.hint.heatmap[i, batch.pseudo_hint_index[i]] = heatmap
